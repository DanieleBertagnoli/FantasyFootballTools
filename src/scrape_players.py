...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, help="Percorso del JSON da creare (predefinito: instance/player_catalogue.json).")
    arguments = parser.parse_args()

    app = Flask(__name__)
    configured_path = os.environ.get("PLAYER_CATALOGUE_PATH")
    output_path = arguments.output or (
        Path(configured_path) if configured_path else Path(__file__).resolve().parent / "instance" / "player_catalogue.json"
    )
    app.config.from_mapping(
        PLAYER_CATALOGUE_PATH=str(output_path),
